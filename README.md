...
To update the database use this command from the Web project folder (replace `Symmetry.Sales.ChatBot` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../Symmetry.Sales.ChatBot.Infrastructure/Symmetry.Sales.ChatBot.Infrastructure.csproj -s Symmetry.Sales.ChatBot.Web.csproj
```
